...
function workspaceLocalDependencyNames(pkgJson) {
  ...
  for (const [name, spec] of dependencyEntries(pkgJson, false)) {
    if (
      typeof spec === "string" &&
      (spec.startsWith("file:") || spec.startsWith("workspace:"))
    ) {
      names.add(name);
    }
  }
...
function buildNoticeText(entries, displayName, productionOnly, recursive, pkgRoot) {
  const summaryLines = summaryCounts(entries).map(
    ([license, count]) => `- ${license}: ${count} package(s)`,
  );
  ...
  const licenseBlocks = [...byHash.values()]
    ...
    .sort(
      (a, b) =>
        ...
        a.packages[0].localeCompare(b.packages[0], undefined, {
          numeric: true,
          sensitivity: "base",
        }),
    )
...
function resolveExcludedNames(pkgJson, extraExcludedPackageNames) {
  return new Set([
    pkgJson.name,
    ...workspaceLocalDependencyNames(pkgJson),
...
async function loadLicenseChecker() {
  try {
    const mod = await import("license-checker-rseidelsohn");
    const checker = mod.default ?? mod;
    return checker.init ?? checker.default?.init;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Missing notice generator dependency: license-checker-rseidelsohn.\nInstall it with: npm install --save-dev license-checker-rseidelsohn\nOriginal error: ${message}`,
    );
  }
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const initLicenseChecker = await loadLicenseChecker();
  if (typeof initLicenseChecker !== "function") {
    throw new TypeError("license-checker-rseidelsohn did not expose init()");
  }
  await generateNotice({ pkgRoot: process.cwd(), initLicenseChecker });
}
